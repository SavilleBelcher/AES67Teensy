...
void l3PTP::sendPTPMessage(const uint8_t *buf, int size, bool generalMessage){
    if(generalMessage){
        generalSocket->beginPacket(adr, generalPort);
        generalSocket->setPacketDiffServ(46);
        generalSocket->write(buf, size);
        generalSocket->endPacket();
    }else{
        eventSocket->beginPacket(adr, eventPort);
        eventSocket->setPacketDiffServ(46);
        eventSocket->write(buf, size);
        eventSocket->endPacket();
    }
}
