...
void PTPBase::reset()
{
    ...
    memcpy(localAnnounce.clockIdentity, clockID, 8);
    localAnnounce.priority1 = clockPriority;     // Or lower if you want to force GM
    localAnnounce.priority2 = clockPriority;
    localAnnounce.clockClass = 248;    // Indicates "ordinary clock"
    localAnnounce.clockAccuracy = 0x21; // 20 = better than 1 µs, 21 = better than 10 µs, fe = Accuracy unknown
    localAnnounce.offsetScaledLogVariance = 0xFFFF;
    memcpy(localAnnounce.grandmasterIdentity, clockID, 8);
    localAnnounce.stepsRemoved = 0;
}
...
void PTPBase::updateController()
{
    ...
    double currentDriftNsps = (1.0 - currentDrift) * NS_PER_S;
    
    // filter out large sporadic sync cycles once locked  
    if (lockcount > 10 ){

         if (consecutiveBadCount > 9){
            lockcount = 0;
            consecutiveBadCount = 0;
            //badSyncCount = 0;
             if (logging){
                Serial.printf("Sporadic Sync Reset\nNo controller update. ConsecutiveBadCount:%d \n", consecutiveBadCount);
        ...
        if (abs(currentDriftNsps) > sporadicSyncErrorThreshold){
            //currentDriftNsps = 0;
            //currentOffset = 0;
            badSyncCount++;
            consecutiveBadCount++;
            if (logging){
                Serial.printf("Sporadic Sync Error\nNo controller update. ConsecutiveBadCount:%d \n", consecutiveBadCount);
                Serial.printf("Delay:%dns ", (int)currentDelay);
                Serial.printf("Offset:%dns ", (int)currentOffset);
                Serial.printf("Drift:%dns \n\n", (int)currentDriftNsps);
            }
    ...
    const bool freqMode = currentDriftNsps > 3000 || currentDriftNsps < -3000;
    
    const bool coarseMode = currentOffset > 50000 || currentOffset < -50000;
    ...
    if(!driftError){
        ...
        if(!freqMode && !coarseMode && currentOffset < 1000 && currentOffset > -1000){
        	lockcount++;
        }else{
        	//lockcount=0;
        }
    ...
    else
    {
        ...
        driftErrorCount++;
        if (driftErrorCount > 50){
            //qindesign::network::EthernetIEEE1588.offsetTimer(offsetCorrection);
            nspsAccu = 0;
            //t2 += offsetCorrection;
            timespec t;
            nanoTimeToTimespec(t4,t);
    ...
    if (logging)
    {
        ...
        Serial.printf("Offset:%dns ", (int)currentOffset);
        Serial.printf("Drift:%dns \n", (int)currentDriftNsps);
        //Serial.printf("Drift:%dns \n", (int)filteredDriftNsps);

        if(driftError)
        {
            Serial.printf("Drift Error\n No controller update. Drift Error Count %d\n",driftErrorCount);
        }
        else if (coarseMode)
        {
...
{
    PTPAnnounceMessage remote;
    memcpy(remote.clockIdentity, &buf[20], 8); // Example offset for ClockIdentity
    remote.priority1 = buf[47];               // Offset from IEEE 1588-2008 Announce
    remote.clockClass = buf[48];
    remote.clockAccuracy = buf[49];
    remote.offsetScaledLogVariance = ((uint16_t)buf[50] << 8) | buf[51];
    remote.priority2 = buf[52];
    memcpy(remote.grandmasterIdentity, &buf[53], 8);
    remote.stepsRemoved = ((uint16_t)buf[45] << 8) | buf[46];

    updateBMCA(remote);

}

bool PTPBase::isBetterClock(const PTPAnnounceMessage& a, const PTPAnnounceMessage& b) 
{
    if (a.priority1 != b.priority1) return a.priority1 < b.priority1;
    if (a.clockClass != b.clockClass) return a.clockClass < b.clockClass;
    if (a.clockAccuracy != b.clockAccuracy) return a.clockAccuracy < b.clockAccuracy;
    if (a.offsetScaledLogVariance != b.offsetScaledLogVariance)
        return a.offsetScaledLogVariance < b.offsetScaledLogVariance;
    if (a.priority2 != b.priority2) return a.priority2 < b.priority2;
    return memcmp(a.clockIdentity, b.clockIdentity, 8) < 0;
}

void PTPBase::updateBMCA(const PTPAnnounceMessage& incoming)
{
    if (isBetterClock(incoming, localAnnounce)) {
        isGrandmaster = false;
        bestAnnounce = incoming;
        if (logging){
            Serial.println("[BMCA] Remote clock is better. Acting as SLAVE.");
        }
        
    } else {
        isGrandmaster = true;
        bestAnnounce = localAnnounce;
        if (logging){
            Serial.println("[BMCA] Local clock is best. Acting as MASTER.");
        }
    }
}
